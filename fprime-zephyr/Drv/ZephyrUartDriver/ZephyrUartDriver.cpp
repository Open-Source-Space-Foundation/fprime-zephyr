...
namespace Zephyr {
    ...
    void ZephyrUartDriver::configure(const struct device *dev, U32 baud_rate) {
        ...
        // Set up the TX staging ring buffer and its dedicated writer thread. This keeps
        // send_handler() (called synchronously by the caller, e.g. the ComAggregator consumer
        // thread) from ever blocking in uart_poll_out(), which can spin indefinitely while a
        // CDC-ACM host session is attached-but-stalled (see cdc_acm_poll_out()).
    ...
    {
        // NOTE: this handler must never block. It is called synchronously on the caller's
        // thread (e.g. the ComAggregator consumer thread draining its own internal queue), and
        // the real UART write (uart_poll_out) can block for a long time - up to the full
        // duration of a stalled CDC-ACM host session (~25s to minutes) - if attempted directly
        // here. Instead, stage a length-prefixed copy of the frame into a small ring buffer and
        // let the dedicated TX writer thread (txWriterTaskEntry) perform the actual write. If
        // there isn't room (sustained TX backpressure), drop the frame rather than block.
            ...
            while (ring_buf_size_get(&self->m_tx_ring_buf) >= sizeof(U32)) {
                ...
                while (remaining > 0) {
                    ...
                    for (U32 i = 0; i < chunkGot; i++) {
                        // This may block for a long time under sustained CDC-ACM backpressure.
                        // That is fine here: this thread is fully decoupled from send_handler's
                        // caller, so it does not stall the aggregator's queue-draining thread.
